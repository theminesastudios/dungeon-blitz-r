  ...
  for (let index = 2; index < argv.length; index += 1) {
    ...
    if (arg === "--help" || arg === "-h") {
      console.log([
        ...
        "BitmapData allocations on wide screens are reduced to a 1x1 transparent",
        "bitmap instead of crashing Flash.",
      ].join("\n"));
      process.exit(0);
    }
...
function dimensionGuard(
  ...
  const guard: InsertedInstruction[] = [
    ...
    getLocal(widthLocal),
    { opcode: 0xab }, // equals
    { opcode: 0x12, branchTo: "invalid" }, // iffalse
    getLocal(heightLocal),
    getLocal(heightLocal),
    { opcode: 0xab },
    { opcode: 0x12, branchTo: "invalid" },
    getLocal(widthLocal),
    { opcode: 0x24, operands: [["s8", 1]] }, // pushbyte 1
    { opcode: 0xad }, // lessthan
    { opcode: 0x11, branchTo: "invalid" }, // iftrue
    getLocal(heightLocal),
    { opcode: 0x24, operands: [["s8", 1]] },
    { opcode: 0xad },
    { opcode: 0x11, branchTo: "invalid" },
    getLocal(widthLocal),
    { opcode: 0x25, operands: [["u30", 8191]] }, // pushshort 8191
  ...
  if (totalPixelsIntIndex !== null) {
    guard.push(
      ...
      { opcode: 0x2d, operands: [["u30", totalPixelsIntIndex]] }, // pushint 16777215
      { opcode: 0xaf },
      { opcode: 0x12, branchTo: "ok" },
    );
  } else {
    guard.push({ opcode: 0x10, branchTo: "ok" }); // jump
  }
...
function rectDimensionGuard(
  ...
  return dimensionGuard(widthLocal, heightLocal, [
    getLocal(rectLocal),
    { opcode: 0x24, operands: [["s8", 1]] },
    { opcode: 0x61, operands: [["u30", widthName]] }, // setproperty width
    getLocal(rectLocal),
    { opcode: 0x24, operands: [["s8", 1]] },
    { opcode: 0x61, operands: [["u30", heightName]] }, // setproperty height
  ], totalPixelsIntIndex);
}
