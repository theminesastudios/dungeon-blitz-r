...
function dumpMethod(cls: string, method: string, onlyOffsets: Array<[number, number]> | null): void {
  const classIdx = classIndexByName(abc, cls)!;
  const inst = abc.instances[classIdx];
  const traits = [...inst.traits, ...(abc.classTraits[classIdx] ?? [])];
  const methodIdx = methodIdxForTrait(traits, abc, method)!;
  ...
  console.log(`\n=== ${cls}.${method} (var_76 idx ${var76}) ===`);
  for (const inst of insts) {
    const isVar76 =
      (inst.opcode === 0x66 || inst.opcode === 0x61 || inst.opcode === 0x68) &&
      inst.operands[0]?.[0] === "u30" &&
      inst.operands[0][1] === var76;
    if (onlyOffsets && !onlyOffsets.some(([a, b]) => inst.offset >= a && inst.offset <= b)) continue;
    if (!onlyOffsets || isVar76) {
...
dumpMethod("BuffType", "method_553", null);
