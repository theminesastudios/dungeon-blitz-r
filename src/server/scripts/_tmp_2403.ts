...
const var2403 = names.indexOf("var_2403");
console.log("var_2403 multiname idx:", var2403);
...
for (const o of owners) {
  ...
  for (const ins of insts) {
    if ((ins.opcode === 0x66 || ins.opcode === 0x61 || ins.opcode === 0x68) && ins.operands[0]?.[0] === "u30" && ins.operands[0][1] === var2403) {
