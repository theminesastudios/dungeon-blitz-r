...
const classIdx = classIndexByName(abc, "CombatState")!;
const inst = abc.instances[classIdx];
const traits = [...inst.traits, ...(abc.classTraits[classIdx] ?? [])];
const methodIdx = methodIdxForTrait(traits, abc, "method_322")!;
const body = abc.methodBodies.get(methodIdx)!;
const code = ctx.body.subarray(body.codeStart, body.codeStart + body.codeLen);
const insts = disassemble(code, "CombatState.method_322");
...
console.log(`CombatState.method_322: ${body.codeLen} bytes, ${insts.length} insts`);
