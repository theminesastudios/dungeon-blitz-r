...
const classIdx = classIndexByName(abc, "class_58")!;
const inst = abc.instances[classIdx];
const traits = [...inst.traits, ...(abc.classTraits[classIdx] ?? [])];
const methodIdx = methodIdxForTrait(traits, abc, "method_1505")!;
const body = abc.methodBodies.get(methodIdx)!;
const code = ctx.body.subarray(body.codeStart, body.codeStart + body.codeLen);
const insts = disassemble(code, "class_58.method_1505");
...
const OP: Record<number, string> = {
  ...
  0x53: "applytype", 0x5d: "findpropstrict", 0x5e: "findproperty", 0x5f: "finddef",
  0x60: "getlex", 0x61: "setproperty", 0x62: "getlocal", 0x63: "setlocal",
...
console.log(`class_58.method_1505: ${body.codeLen} bytes`);
for (const inst of insts) {
  if (inst.offset >= 200 && inst.offset <= 340) console.log(fmt(inst));
}
