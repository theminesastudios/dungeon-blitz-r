...
console.log("== strings containing Poison/Dagger/Status ==");
abc.stringValues.forEach((s, i) => {
  if (/Poison|Dagger/i.test(s) || /StatusIcon/.test(s)) console.log(`  [${i}] ${JSON.stringify(s)}`);
});
...
console.log(`\nBuffType.method_1811: ${body.codeLen} bytes, ${insts.length} insts`);
