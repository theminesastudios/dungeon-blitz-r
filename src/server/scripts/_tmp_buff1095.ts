...
const classIdx = classIndexByName(abc, "Buff")!;
const inst = abc.instances[classIdx];
const traits = [...inst.traits, ...(abc.classTraits[classIdx] ?? [])];
const methodIdx = methodIdxForTrait(traits, abc, "method_1095")!;
const body = abc.methodBodies.get(methodIdx)!;
const code = ctx.body.subarray(body.codeStart, body.codeStart + body.codeLen);
const insts = disassemble(code, "Buff.method_1095");

const var76 = names.indexOf("var_76");
console.log("var_76 multiname idx:", var76, "Buff.method_1095:", body.codeLen, "bytes,", insts.length, "insts");
...
// print windows around var_76 getproperty/setproperty
...
insts.forEach((inst, i) => {
  if (inst.opcode === 0x66 && inst.operands[0]?.[0] === "u30" && inst.operands[0][1] === var76) {
    windows.push([Math.max(0, i - 10), Math.min(insts.length, i + 15)]);
  }
});
for (const [from, to] of windows) {
  console.log("\n-- window --");
  for (let i = from; i < to; i += 1) console.log("  " + fmt(insts[i]));
}
